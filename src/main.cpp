...
class Puck {
  public:
    ...
    int movy;
    int r;
    void drawcric() { DrawCircle(x, y, r, color); }
    void newpos() {
        if (x >= WIDTH - r) {
            movx *= -1;
        }
...
void send_to_peer(int socket_fd, float *buffer,
                  const std::string &other_client_ip) {
    sockaddr_in peer_addr;
    peer_addr.sin_family = AF_INET;
    inet_pton(AF_INET, other_client_ip.c_str(), &peer_addr.sin_addr);
...
void receive_from_peer(int socket_fd, float *buffer,
    ...
    inet_pton(AF_INET, other_client_ip.c_str(), &sender_addr.sin_addr);
...
int main() {
    ...
    // Input other client's IP address
    std::string other_client_ip;
    std::cout << "Enter the IP address of the other client: ";
    std::cin >> other_client_ip;

    // Sender connection
    channel.emplace_back(send_to_peer, sender_socket, buf, other_client_ip);
    channel.back().detach();
    ...
    while (!WindowShouldClose()) {
        ...
        if (CheckCollisionCircleRec(
                Vector2{(float)ball.x, (float)ball.y}, ball.r,
        ...
        // DrawRectangle(WIDTH-15, HEIGHT/2-PADDLEH/2, PADDLEW, PADDLEH,
        // RAYWHITE);
